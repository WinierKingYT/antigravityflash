...
def register_runtime_context(
    ...
    """Appends a cryptographically verified context event to .agent-harness/context-registry.jsonl."""
    ...
    # Read last record to obtain previousHash
...
def evaluate_context_isolation(
    ...
    if origin in {"MOCK", "SIMULATED_INTEGRATION"} and not allow_simulated:
        return "CONTEXT_ISOLATION_NOT_PROVEN", {
            "status": "FAIL",
            "reason": f"Simulated context origin '{origin}' rejected in production verification mode",
        }
