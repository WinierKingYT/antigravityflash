...
class ReleaseConsistencyTestSuite(unittest.TestCase):
    def test_version_alignment(self):
        """Verify version is 1.0.1 across pyproject.toml, package __init__, installer, and README."""
        # pyproject.toml
        ...
        self.assertEqual(pyproject_ver, "1.0.1")
        ...
        self.assertEqual(match_init.group(1), "1.0.1")

        # scripts/install.py
        install_text = (REPO_ROOT / "scripts" / "install.py").read_text(encoding="utf-8")
        self.assertIn("V1.0.1", install_text)
        self.assertNotIn("V1.0.0", install_text)

        # README.md
        readme_text = (REPO_ROOT / "README.md").read_text(encoding="utf-8")
        self.assertIn("V1.0.1", readme_text)
        self.assertIn("badge/version-1.0.1-blue.svg", readme_text)
